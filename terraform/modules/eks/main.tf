...
resource "aws_security_group" "node_group" {
  ...
  ingress {
    description = "All traffic from node group"
    from_port   = 0
    to_port     = 0
    protocol    = "-1"
    self        = true
...
# Allow communication between cluster and node group
resource "aws_security_group_rule" "cluster_to_node" {
  description              = "Allow cluster to communicate with node group"
  type                     = "ingress"
  from_port                = 0
  to_port                  = 65535
  protocol                 = "tcp"
...
# Allow bastion host to access EKS cluster API
...
# Allow bastion host to access worker nodes
resource "aws_security_group_rule" "bastion_to_nodes" {
  count                    = var.enable_bastion_access ? 1 : 0
  description              = "Allow bastion host to access worker nodes"
  type                     = "ingress"
  from_port                = 0
  to_port                  = 65535
  protocol                 = "tcp"
...
resource "aws_eks_node_group" "main" {
  ...
  tags = {
    ...
    Project     = var.project_name
    Environment = var.environment
    # Tags required for cluster autoscaler auto-discovery
    ...
    Statement = [
      {
        ...
        Condition = {
          StringEquals = {
            "${replace(aws_iam_openid_connect_provider.cluster[0].url, "https://", "")}:sub" = "system:serviceaccount:kube-system:efs-csi-controller-sa"
            "${replace(aws_iam_openid_connect_provider.cluster[0].url, "https://", "")}:aud" = "sts.amazonaws.com"
          }
        }
      }
...
resource "aws_eks_addon" "efs_csi_driver" {
  ...
  resolve_conflicts = "OVERWRITE"
  service_account_role_arn = var.enable_irsa ? aws_iam_role.efs_csi_driver[0].arn : null

  depends_on = [aws_eks_node_group.main]

  tags = {
    Name        = "${var.cluster_name}-efs-csi-driver-addon"
    Project     = var.project_name
    Environment = var.environment
  }
}
